...
# Host part.
###############################################################################
ifdef ALCHEMY_SUPPORT_HOST_MODULE
include $(CLEAR_VARS)

LOCAL_HOST_MODULE := zlib

LOCAL_AUTOTOOLS_VERSION := 1.2.3
LOCAL_AUTOTOOLS_ARCHIVE := $(LOCAL_HOST_MODULE)-$(LOCAL_AUTOTOOLS_VERSION).tar.bz2
LOCAL_AUTOTOOLS_SUBDIR := $(LOCAL_HOST_MODULE)-$(LOCAL_AUTOTOOLS_VERSION)

# Override install prefix
LOCAL_AUTOTOOLS_MAKE_INSTALL_ARGS := \
	prefix=$(HOST_OUT_STAGING)/usr
...
# Remove some installed files after clean
define LOCAL_AUTOTOOLS_CMD_POST_CLEAN
	$(Q) rm -f $(HOST_OUT_STAGING)/usr/include/zlib.h
	$(Q) rm -f $(HOST_OUT_STAGING)/usr/include/zconf.h
endef

include $(BUILD_AUTOTOOLS)
endif # ALCHEMY_SUPPORT_HOST_MODULE
